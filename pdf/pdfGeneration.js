    ...
    participants.rank(participant.start_number).then(rank => {
      participants.getTime(participant.start_number).then(time => {
        if (_.isNull(time)) {
          ...
          doc.fontSize(35).fillColor('black').text(participant.firstname.replace(/\|/g, '\n') + ' ' + participant.lastname.substring(0, 30), 0, 370, {align: 'center'});
          doc.fontSize(25).fillColor('black').text(participant.team.substring(0, 30), 0, 410, {align: 'center'});
          doc.fontSize(30).fillColor('black').text(timestring, 0, 490, {align: 'center'});
          doc.fontSize(40).fillColor('black').text(rank, 0, 555, {align: 'center'});
          doc.fontSize(40).fillColor('black').text(category_rank, 0, 625, {align: 'center'});
          deferred.resolve();
        }
        ;
      });
